...
class ImageRecognitionResponse(BaseModel):
    ...
    status: Literal["success", "need_label_photo"] = Field(..., description="Recognition status")
    manufacturer: ManufacturerName = Field(..., description="Manufacturer name")
    model_number: Optional[str] = Field(None, description="Model number (null if not readable)")
    category: str = Field(..., description="Product category")
    is_new_category: bool = Field(False, description="Whether the category is newly suggested by AI")
    confidence: Literal["high", "medium", "low"] = Field(..., description="Confidence level")
    label_guide: Optional[LabelGuide] = Field(None, description="Guide if label not readable")
    error: Optional[str] = Field(None, description="Error message if failed")
    raw_response: Optional[str] = Field(None, description="Raw LLM response if JSON parse failed")
...
class ManualSearchRequest(BaseModel):
    ...
    official_domains: Optional[list[str]] = Field(
        None,
        description="List of official domains to search (e.g., ['hitachi.co.jp'])"
    )
...
class ManualSearchResponse(BaseModel):
    ...
    pdf_url: Optional[str] = Field(None, description="URL of the PDF if found")
...
class MaintenanceItem(BaseModel):
    ...
    description: str = Field(..., description="Detailed description")
    frequency: str = Field(..., description="Maintenance frequency (e.g., '週1回', '月1回')")
    ...
    page_reference: Optional[str] = Field(None, description="Reference page in manual")
...
class MaintenanceExtractionRequest(BaseModel):
    ...
    manufacturer: Optional[str] = Field(None, description="Manufacturer name (optional)")
    model_number: Optional[str] = Field(None, description="Model number (optional)")
...
class MaintenanceExtractionResponse(BaseModel):
    ...
    maintenance_items: list[MaintenanceItem] = Field(..., description="List of maintenance items")
