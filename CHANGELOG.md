...
**バックエンド**
- `qa_chat_service.py`: 各処理にタイムアウト設定
  - QA検索: 30秒
  - テキストキャッシュ検索: 60秒
  - PDF直接分析: 90秒
  - セルフチェック: 30秒
- `text_cache_service.py`: PDFテキスト抽出に120秒タイムアウト
- `QAStreamEvent`: `step_timeout` イベント追加、`message` フィールド追加
- タイムアウト時の処理
  - 次のフォールバック検索へ自動移行
  - セルフチェックタイムアウト時は検証スキップして回答表示
  - 全ステップタイムアウト時はエラーメッセージ表示

**フロントエンド**
- `SearchProgressIndicator.tsx`: タイムアウト状態の視覚的フィードバック
  - 黄色/オレンジのインジケーター表示
  - タイムアウトメッセージ表示
  - ステップアイコンに時計アイコン表示
- `QAChat.tsx`: `timedOutSteps` 追跡
- `qa.ts`: `SearchProgress` に `timedOutSteps` と `timeoutMessage` 追加

**UI/UX**
- タイムアウト時も処理を継続し、ユーザーを待たせすぎない設計
- タイムアウト発生時の明確な視覚的フィードバック

#### 登録ページ詳細モーダル改善 ✅ 完了

- `/register` ページの詳細モーダルにスクロール機能追加
- 長い説明文でもモーダルが画面からはみ出さない

#### pre-commitフック強化 ✅ 完了

コード品質とセキュリティを向上させるため、pre-commitフックを大幅に強化。

**新しいチェック項目**
- **TypeScript型チェック**: `tsc --noEmit` による型エラー検出（frontendディレクトリ対象）
- **秘密情報漏洩防止**: `detect-secrets` + `detect-private-key` による包括的な秘密情報検出
- **デバッグコード検出**: `print()` / `console.log` の自動検出（`.claude/hooks/no-debug-code.py`）
- **コミットメッセージ規約**: Conventional Commits形式の強制（`feat:`, `fix:`, `docs:` 等）

**追加ファイル**
- `.pre-commit-config.yaml`: 4つの新しいフック設定を追加
- `.claude/hooks/no-debug-code.py`: デバッグコード検出スクリプト
- `.secrets.baseline`: detect-secretsのベースラインファイル（誤検知除外用）
- `.git/hooks/commit-msg`: コミットメッセージフック

**技術的特徴**
- ベースライン方式でドキュメント内のサンプルコード等の誤検知を除外
- テストファイル（`test_*`, `*.test.ts` 等）はデバッグコード検出の対象外
- `pre-commit install --hook-type commit-msg` でコミットメッセージフックを有効化

#### トップページ: メンテナンス詳細モーダル追加 ✅ 完了

トップページのメンテナンス一覧から詳細情報を確認・編集できる機能を追加。

**フロントエンドUI**
- `/` ページ（トップページ）: メンテナンス詳細モーダル
  - メンテナンス項目クリックで詳細モーダルを表示
  - 説明文、周期、重要度、参照ページを表示
  - 次回予定日の編集機能（カレンダーUI）
  - 完了履歴の表示機能
  - 「完了する」ボタンから完了モーダルへ遷移
  - 家電詳細ページへのリンク
- `MaintenanceListItem.tsx`: コンパクト版でもクリックイベントをサポート

**UI/UX改善**
- メンテナンス一覧ページ・トップページの次回予定日フォームを簡素化
  - 不要な例表示（"例: 2025-06-15"）を削除
  - 選択中日付のテキスト表示を削除
  - 説明文を削除してシンプルに
...
#### PDFビューア機能（react-pdf）✅ 完了

iOS Safari / PWA でも説明書PDFを快適に閲覧できるカスタムPDFビューア。
...
- タッチジェスチャー
  ...
  - パン（ドラッグ）で拡大時の移動
  - 倍率に応じたスワイプ/パン切り替え
- CJKフォントサポート（日本語・中国語・韓国語の正常表示）
- JPEG 2000対応（一部PDFで使用される画像形式）
- 暗号化PDFのフォールバック（ブラウザ表示へ誘導モーダル）

**技術的特徴**
- `@/components/pdf/PDFViewer.tsx` - タッチジェスチャー対応PDFビューアコンポーネント
- `/pdf-viewer` ページ - クエリパラメータでPDF URL・初期ページを指定
- pdf.js worker・CMapsはVercelで公開済みの静的ファイルを参照
- ブラウザズーム防止（`touch-action: none`）
...
- **お問い合わせフォーム送信後のスクロール位置リセット**: フォーム送信完了後に画面上部にスクロールするよう修正（PR #23）
- **メンテナンス一覧からPDFをreact-pdfビューアで開くよう修正**: ブラウザネイティブビューアではなく `/pdf-viewer` ページで開くよう変更。署名URL事前取得の削除によるパフォーマンス改善（PR #24）
- **操作後の画面更新問題を修正**: SWRの`dedupingInterval`による古いデータ表示問題を `window.location.reload()` で解決。メンテナンス完了、家電削除、登録完了、グループ操作（招待コード再生成、メンバー削除、グループ名編集）の7箇所を修正（PR #25）
