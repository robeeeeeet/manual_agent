...
| 表示名 | ユーザープロファイルに表示名を設定可能 |
| 家族グループ共有 | 招待コード方式でグループ共有 |
| マイページ | プラン・利用状況、メンテナンス統計、通知設定、通知時刻変更、ログアウト |

#### マイページ機能

| 機能 | 詳細 |
|------|------|
| プラン・利用状況 | 現在のティア、登録家電数、説明書検索回数(日)、QA質問回数(日) |
...
| ティア | 家電登録上限 | 説明書検索/日 | QA質問/日 |
|--------|------------|--------------|-----------|
| free（無料） | 10台 | 5回 | 10回 |
| premium（将来） | 無制限 | 無制限 | 無制限 |
...
### 実装済み
- [x] **家電詳細画面 + AIエージェント質問機能**（QAマークダウン方式、3段階フォールバック検索）
- [x] **家族グループ共有機能**（招待コード方式、メンバー管理）
- [x] **商品ごとのQAドキュメント自動生成**（QAマークダウンファイル生成）
- [x] **QA会話履歴機能**（セッション管理、タイトル自動生成）
- [x] **パスワードリセット機能**
- [x] **ユーザーティア機能**（利用制限、使用量トラッキング）
- [x] **ヘルプページ**（使い方ガイド、FAQ、トラブルシューティング）
- [x] **パナソニック製品の公式サイト直接検索**
...
daily_usage (日次使用量)
├── id, user_id, date
├── manual_search_count  # 説明書検索回数
├── qa_question_count    # QA質問回数

qa_sessions (QAセッション)
├── id, user_id, shared_appliance_id
├── title, messages, is_active
```
...
| `skip_domain_filter` | （現在はデフォルトで無効化済み）ドメインフィルターを無効化し、より広範囲に検索 |
...
- `success_count` で成功回数を追跡
- **注**: 現在、検索時のドメインフィルタリング（`site:domain`）は無効化されており、全ドメインから検索する。ドメイン登録は将来の利用に備えて継続している
