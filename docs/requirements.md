...
- [ ] 家電の写真（ラベル含む）をGeminiに送信
- [ ] メーカー名・型番を正確に抽出できるか検証
- [ ] 外観から商品を推測し、ラベル位置を指示できるか検証
- [ ] 複数の家電カテゴリでテスト（エアコン、洗濯機、冷蔵庫など）

#### 0-2. メーカー・型番からマニュアルPDF取得
- [ ] メーカー名・型番からWeb検索でPDFを見つけられるか
- [ ] Gemini + Web検索の組み合わせで自動取得が可能か
- [ ] 取得成功率の検証（主要メーカー）

#### 0-3. マニュアルからメンテナンス項目抽出
- [ ] PDFをGeminiに読み込ませてメンテナンス項目を抽出
- [ ] 周期（月1回、年1回など）を正確に抽出できるか
- [ ] 複数のマニュアルでテスト
...
1. この要件定義の内容を確認・承認
2. **Phase 0（フィジビリティ確認）から開始**
   - AI機能が実現可能かを先に検証
   - Go/No-Goを判断してからPhase 1へ進む
