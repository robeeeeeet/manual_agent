...
#### PDFページ番号分離機能 ✅ 完了
- [x] `shared_maintenance_items` のページ番号カラム分離
  - [x] `pdf_page_number` (INTEGER): PDFビューアで表示されるページ番号
  - [x] `printed_page_number` (TEXT): 説明書に印刷されているページ番号
- [x] マイグレーション 00019, 00020
- [x] Gemini APIによる既存データの自動マイグレーション
- [x] バックエンド/フロントエンド対応

#### PDFビューア機能（react-pdf）✅ 完了
iOS Safari / PWA でも説明書PDFを快適に閲覧できるカスタムPDFビューア。

- [x] react-pdf + pdf.js による高品質レンダリング
- [x] ピンチズーム（2本指の中心点を基準に拡大縮小）
- [x] タッチジェスチャー
  - [x] スワイプでページ移動
  - [x] パン（ドラッグ）で拡大時の移動
  - [x] 倍率に応じたスワイプ/パン切り替え
- [x] CJKフォントサポート（日本語・中国語・韓国語の正常表示）
- [x] JPEG 2000対応（一部PDFで使用される画像形式）
- [x] 暗号化PDFのフォールバック（ブラウザ表示へ誘導モーダル）
- [x] PDFビューアページ（`/pdf-viewer`）

#### 購入日機能と次回予定日編集機能 ✅ 完了
家電の購入日を記録し、メンテナンス予定日を購入日から起算できる機能。

- [x] `user_appliances.purchased_at` カラム追加（DATE型）
- [x] マイグレーション `20260113000001_add_purchased_at.sql`
- [x] 購入日起算のメンテナンス日計算ロジック
  - [x] `_calculate_next_due_from_purchase()` - 過去の購入日から将来の次回日まで繰り返し計算
  - [x] `dateutil.relativedelta` による正確な月次計算（月末境界対応）
- [x] `PATCH /api/v1/maintenance/{schedule_id}/next-due` API
- [x] カレンダーUIで次回予定日を直接編集
- [x] 家電登録ページ（Step 2）に購入日入力フィールド追加
...
- ✅ **PDFページ番号分離**: PDFビューア用ページ番号と印刷ページ番号を分離管理
- ✅ **PDFビューア機能**: react-pdf + pdf.js、ピンチズーム、タッチジェスチャー、CJKフォント対応
- ✅ **購入日機能と次回予定日編集**: 購入日起算のメンテナンス日計算、カレンダーUI
- ✅ **QA回答セルフチェック**: LLMによる回答品質の自動検証
