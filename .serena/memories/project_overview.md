...
6. **認証フロー**: @supabase/ssr + ミドルウェアによるルート保護、サインアップ時はOTPコード方式（PWA対応のためメールリンク方式から変更）、確認コード再送機能（Supabaseレート制限対応・日本語カウントダウン表示）、未認証ユーザーは全保護ルート（`/`, `/appliances`, `/register`, `/mypage`）からログインページへリダイレクトされ、ログイン後は元のページに戻る（`redirectTo`クエリパラメータ）
